...
#sends the Clear Passcode command to all devices in an advanced search
...
deviceNumbers=($(/usr/bin/curl -X GET -H "Accept: application/xml" -s -u "${jssUser}":"${jssPassword}" ${jssURL}/JSSResource/advancedmobiledevicesearches/id/"$searchID" | xpath -e "//id" 2> /dev/null | awk -F'</?id>' '{for(i=2;i<=NF;i++) print $i}'))

clearPasscode(){

goodXML="<mobile_device_command>
    <general>
        <command>ClearPasscode</command>
    </general>
    <mobile_devices>
        <mobile_device>
            <id>$device</id>
...
curl -X POST \
  ${jssURL}/JSSResource/mobiledevicecommands/command/ClearPascode \
...
for device in "${deviceNumbers[@]}"; do
	#clearPasscode
  echo "Passcode cleared on device number $device"
done
