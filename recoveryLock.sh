...
#This script sets a recovery lock on an advanced search of computers through the Jamf Pro API
#Before running, make sure you have created an advanced search of 2000 or fewer computers and found the ID number
#You will also need jq installed to parse the JSON that the API returns
#More details can be found on my blog: https://gingerscripting.com/setting-an-apple-silicon-recovery-lock-password-through-the-jamf-api/
...
computers=($(curl -s "${jamfURL}/JSSResource/advancedcomputersearches/id/$searchNumber" -H "Accept: application/json" -H "Authorization: Bearer ${token}" -X GET | jq ".advanced_computer_search.computers[].id" ))
	for computerID in "${computers[@]}"; do
		...
		echo "Recovery Lock enabled for computer number $computerID."
	done
...
echo "Complete. All computers in the advanced search have had recovery lock enabled."
